...
function repeatString(str, times) {
  return times > 0 ? str.repeat(times) : str.repeat(0);
}
...
function sumOfCodes(str) {
  return typeof str === 'string'
    ? str
        .split('')
        .map((currentValue) => currentValue.charCodeAt(0))
...
function countVowels(str) {
  return str.length === 0
    ...
    : str
        .split('')
        .map((currentValue) => {
          if ('aeiouyAEIOUY'.includes(currentValue)) return 1;
          return 0;
        })
        .reduce((accumulator, currentValue) => accumulator + currentValue, 0);
}
...
function isPalindrome(str) {
  const newString = str
    ...
    .replaceAll('?', '')
    .toLowerCase();
  return newString.split('').reverse().join('').includes(newString);
}
...
function findLongestWord(sentence) {
  if (sentence === '') return '';
  const arrayOfLength = sentence
    .split(' ')
    .map((currentValue) => currentValue.length);
  const lengthOfArray = arrayOfLength.length;
  let indexOfMax = 0;
  for (let i = 1; i < lengthOfArray; i += 1) {
    if (arrayOfLength[indexOfMax] < arrayOfLength[i]) indexOfMax = i;
  }
  return sentence.split(' ')[indexOfMax];
}
...
function reverseWords(str) {
  ...
  return str
    .split(' ')
    .map((currentValue) => currentValue.split('').reverse().join(''))
    .join(' ');
}
